...
export default function CreateInvoice() {
  ...
  const invoiceDateDisplay = form.invoiceDate
    ? format(new Date(form.invoiceDate), "MMMM d, yyyy")
        ...
        <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-6 md:p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Bill To (Customer)</h2>
            ...
            {/* Route */}
            <div className="md:col-span-2">
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3 mt-2">Shipment Details</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Origin <span className="text-primary">*</span></label>
                  <Input name="origin" value={form.origin} onChange={handleChange} placeholder="e.g. Kathmandu" className="h-10" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Destination <span className="text-primary">*</span></label>
                  <Input name="destination" value={form.destination} onChange={handleChange} placeholder="e.g. Tokyo" className="h-10" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Weight (kg)</label>
                  <Input name="weight" type="number" value={form.weight} onChange={handleChange} placeholder="e.g. 5" className="h-10" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Cost (NPR) <span className="text-primary">*</span></label>
                  <Input name="cost" type="number" value={form.cost} onChange={handleChange} placeholder="e.g. 15000" className="h-10" />
                </div>
              </div>
            </div>
            ...
            {/* Sender / Receiver */}
            <div className="md:col-span-2">
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3 mt-2">Sender & Receiver</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
          ...
          <div className="flex gap-3 mt-6 pt-6 border-t border-gray-100">
            ...
            <Button variant="outline" onClick={() => { setForm(emptyForm); setShowPreview(false); }}>
              Clear
            </Button>
          </div>
        </div>
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 32, gap: 32 }}>
              <div style={{ flex: 1 }}>
                ...
                {form.billToEmail && (
                  <div style={{ color: GRAY, fontSize: 13, marginTop: 2 }}>{form.billToEmail}</div>
                )}
              </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" as const, marginBottom: 0 }}>
              <thead>
                <tr style={{ background: BLUE, color: "white" }}>
                  ...
                  <th style={{ textAlign: "center" as const, padding: "10px 12px", fontWeight: 600, fontSize: 12, width: 70 }}>Quantity</th>
                  <th style={{ textAlign: "right" as const, padding: "10px 12px", fontWeight: 600, fontSize: 12, width: 110 }}>Rate</th>
                  <th style={{ textAlign: "right" as const, padding: "10px 12px", fontWeight: 600, fontSize: 12, width: 110 }}>Amount</th>
                </tr>
              </thead>
              <tbody>
                <tr style={{ background: LIGHT }}>
                  <td style={{ padding: "12px", fontSize: 13, color: "#212529", verticalAlign: "top" as const }}>
                    {itemDescription}
                  </td>
                  <td style={{ padding: "12px", fontSize: 13, textAlign: "center" as const, color: "#212529" }}>1</td>
                  <td style={{ padding: "12px", fontSize: 13, textAlign: "right" as const, color: "#212529" }}>
                    {formatNPR(amount)}
                  </td>
                  <td style={{ padding: "12px", fontSize: 13, textAlign: "right" as const, color: "#212529", fontWeight: 600 }}>
                    {formatNPR(amount)}
                  </td>
                </tr>
              </tbody>
              ...
              <table style={{ borderCollapse: "collapse" as const, minWidth: 240 }}>
                <tbody>
                  <tr style={{ borderTop: `1px solid ${BORDER}` }}>
                    ...
                    <td style={{ padding: "8px 12px", fontSize: 13, textAlign: "right" as const, color: "#212529" }}>{formatNPR(amount)}</td>
                  </tr>
                  <tr style={{ borderTop: `1px solid ${BORDER}` }}>
                  ...
                  <tr style={{ borderTop: `2px solid ${BLUE}`, background: BLUE }}>
                    ...
                    <td style={{ padding: "10px 12px", fontSize: 14, fontWeight: 700, textAlign: "right" as const, color: "white" }}>{formatNPR(amount)}</td>
                  </tr>
                  {form.paymentTerms === "Payment Received" && (
                    <tr style={{ borderTop: `1px solid ${BORDER}` }}>
                      <td style={{ padding: "8px 12px", fontSize: 13, color: "#198754", fontWeight: 600 }}>Amount Paid</td>
                      <td style={{ padding: "8px 12px", fontSize: 13, textAlign: "right" as const, color: "#198754", fontWeight: 600 }}>{formatNPR(amount)}</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <div style={{ marginTop: 32 }}>
              ...
              <div style={{ fontSize: 13, color: "#212529", whiteSpace: "pre-line" as const, lineHeight: 1.6 }}>
                {notes}
              </div>
            </div>
          </div>
