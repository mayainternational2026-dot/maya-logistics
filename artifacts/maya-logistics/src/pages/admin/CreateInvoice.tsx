...
export default function CreateInvoice() {
  ...
  const InvoiceDoc = () => (
    ...
    >
      ...
      {/* ── SIGNATURE ── */}
      <div style={{ marginTop: 20, paddingTop: 14, borderTop: `2px solid ${BORDER}`, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div style={{ fontSize: 10, color: GRAY, fontStyle: "italic", maxWidth: 200 }}>
          <div style={{ marginBottom: 4 }}>Generated by Maya Import Export Logistic System</div>
          <div>{new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</div>
        </div>
        <div style={{ textAlign: "center", minWidth: 220 }}>
          <div style={{ height: 52, borderBottom: `2px solid #212529`, marginBottom: 6, position: "relative" }}>
            <div style={{ position: "absolute", bottom: 6, left: "50%", transform: "translateX(-50%)", fontSize: 9, color: GRAY, whiteSpace: "nowrap" }}>Sign above this line</div>
          </div>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#212529", letterSpacing: 0.3 }}>Authorized Signatory</div>
          <div style={{ fontSize: 11, color: GRAY, marginTop: 2 }}>{CO.name}</div>
          <div style={{ fontSize: 10, color: GRAY, marginTop: 1 }}>Authorized Representative</div>
        </div>
      </div>
    </div>
  );
